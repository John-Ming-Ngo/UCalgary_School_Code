...
struct BySizeAndAddress {
	bool operator () (std::list<MemAlloc>::iterator lhs, std::list<MemAlloc>::iterator rhs) {
		...
		if ((*lhs).size == (*rhs).size) { //If the files are the same size, sort them by address. Else, sort them by their size.
			return (&(*lhs) < &(*rhs));
		}
...
struct MemSim {
  private:
	...
	std::set<std::list<MemAlloc>::iterator,BySizeAndAddress> freeBlocks;
    ...
    for (std::list<MemAlloc>::iterator it1:taggedBlocks[tag]) {
		...
		if (left != allocations.begin()) {
			...
			if ((*left).tag == FREE) {
				...
				std::pair<std::set<std::list<MemAlloc>::iterator>::iterator,std::set<std::list<MemAlloc>::iterator>::iterator> retVal = freeBlocks.equal_range(left);
				freeBlocks.erase(retVal.first, retVal.second);
		...
		if (right != allocations.end()) { //Might I have exceeded allocations.end and so this thing is broken for that reason? Possibly...
			if ((*right).tag == FREE) {
				...
				std::pair<std::set<std::list<MemAlloc>::iterator>::iterator,std::set<std::list<MemAlloc>::iterator>::iterator> retVal = freeBlocks.equal_range(right);
				freeBlocks.erase(retVal.first, retVal.second);
				allocations.erase((*(retVal.first)));
  ...
  void alloc( long tag, long size) {
	...
	std::pair<std::set<std::list<MemAlloc>::iterator>::const_iterator,std::set<std::list<MemAlloc>::iterator>::const_iterator> findMatch = freeBlocks.equal_range((--allocations.end())); //This is some insane mental gymnastics. TODO: This might not work right anymore.
	...
	if (((*candidate).size == size) && (candidate != (--allocations.end()))) {
		...
		//Remove the candidate from the list of availiable. !!TODO NOT DONE
		//Todo: Check if this remove code makes actual sense. => Can't use this anymore, not using this.
		
		std::pair<std::set<std::list<MemAlloc>::iterator>::iterator,std::set<std::list<MemAlloc>::iterator>::iterator> retVal = freeBlocks.equal_range(candidate);
		freeBlocks.erase(retVal.first, retVal.second);
