...
///
/// compile with:
///   $ g++ 5sumFactorsThreadPool.cpp -O3 -o gcd -lm -lpthread

/**
New attempt to purge mutexes, perhaps use barriers instead.
*/
...
//Implementation 5: Parallelize smallest divisor, thread reuse, NO task pool.
struct DivData {
	int64_t start;
	int64_t end;
	int64_t n;
...
pthread_mutex_t taskWaitMutex;
pthread_cond_t waitTasks;

pthread_mutex_t tasksWaitingMutex;
pthread_cond_t tasksWaiting;
...
void distribute(int64_t low, int64_t high, int64_t increment, int64_t n) {
...
void* getSmallestDivisorThreadCore(void* id) {
  ...
  while (true) {
	  ...
	  if(res == PTHREAD_BARRIER_SERIAL_THREAD) {
		  pthread_mutex_lock(&taskWaitMutex);
		  std::cout<<"Waiting on the start signal."<<"\n";
		  tasksComplete = true;
		  pthread_cond_wait(&waitTasks, &taskWaitMutex); //Wait until all threads are signalled to go. They will go until they hit this point again.
		  pthread_mutex_unlock(&taskWaitMutex);
	  }
	  ...
	  DivData threadData = tasks[threadId];
...
int64_t getSmallestDivisorThreaded(int64_t n, int64_t nThreads)
{
  ...
  distribute(i, max, 6, n); //Distribute the work across the threads. The increment is 6, from the core...
  
  //std::cout << "Single-Threaded component done.\n";
 
  //Todo: Consider a spinlock to ensure all threads make it to the waiting point...
  ...
  } //Wait until tasks are done. TODO: Is it possible that it's toggled before it gets to the point of listening?? Maybe I should use two signals. Huh, was this the problem??
...
int main( int argc, char ** argv)
{
  ...
  pthread_mutex_init(&taskWaitMutex, NULL);
  pthread_mutex_init(&tasksWaitingMutex, NULL);
  ...
  */
  ...
  pthread_mutex_destroy(&taskWaitMutex);
  pthread_mutex_destroy(&tasksWaitingMutex);
