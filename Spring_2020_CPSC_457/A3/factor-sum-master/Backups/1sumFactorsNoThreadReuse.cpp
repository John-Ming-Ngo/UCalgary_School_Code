...
int64_t getSmallestDivisorThreaded(int64_t n, int64_t nThreads)
{
  ...
  DivData threadData[nThreads]; //We need as many parameters as threads to pass in.
  
  distribute(i, max, 6, threadData, n, nThreads); //Distribute the work across the threads. The increment is 6, from the core...
  ...
  for (long i = 0; i < nThreads; i++){// Wait for all of them.
