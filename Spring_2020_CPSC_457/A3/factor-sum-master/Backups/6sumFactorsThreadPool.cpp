...
///
/// compile with:
///   $ g++ 6sumFactorsThreadPool.cpp -O3 -o gcd -lm -lpthread
...
int64_t getSmallestDivisorThreaded(int64_t n, int64_t nThreads, int64_t threadingThreshold);
...
int64_t getSmallestDivisorThreaded(int64_t n, int64_t nThreads, int64_t threadingThreshold)
{
  ...
  if (n < threadingThreshold) {
	  return getSmallestDivisor(n);
  }
...
int main( int argc, char ** argv)
{
  ...
  while( 1) {
    ...
    int64_t divVal = getSmallestDivisorThreaded(num, nThreads, thresholdVal);
