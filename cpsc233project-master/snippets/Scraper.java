...
public class Scraper {
    ...
    public static void main(String[] args) throws IOException {
        ...
        myScraper.scrapePrice("MSFT", "2017-02-01", 7, 52);
 ...
 *         String question = document.select("#question .post-text").text();
		...
		if (isValidDate(startdate, interval, numberOfIntervals)) {
			...
			try {
				...
				//~ for (int count = 0, index_i = lines.length; count < lines.length; count++) {
				...
				//lines[index] has the most recent dates first, we would like the latest(which is the start date first so 
				// we loop through array backwards starting at the first available date
				for (int index_i = lines.length - 1; index_i >= 0 && index_p < numberOfIntervals; index_i--) {
					// pick every 5th line which accounts for 1 pick each week
					// TODO: Account for weekends and holidays???
                    //~ System.out.println("Index I:" + index_i);
                    //~ System.out.println("Index P:" + index_p);
                    //~ System.out.println(lines[index_i]);
					if (lines[index_i] != null) {
						String[] token = lines[index_i].split(",");
						//price[index_p] = Double.parseDouble(token[4]);
       ...
       priceHistory = new double[tempList.size()];
       ...
       for (int count = 0; count < tempList.size(); count++) {
           //System.out.println(tempList.get(count));
					 priceHistory[count] = (double)tempList.get(count);
        }
